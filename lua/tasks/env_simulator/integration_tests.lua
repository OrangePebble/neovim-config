...
local integration_tests = {
	...
	resolve_context = function()
		...
		local targets = utils.run_bazel_query({
			"--output=label_kind",
			"kind(cc_test, //tools/env_simulator/astas_core/Tests/Integration/...)",
		}, ddad_path, co)
		if not targets then
			return nil
		end
		...
		picker.select_one(test_targets, {
			prompt = "Select integration-test target",
		...
		picker.select_many(test_names, {
			prompt = "Select integration test",
		}, function(selected)
		...
		context.context_name = "Run integration tests:" .. table.concat(context.selected_tests, ", ")
		return context
	end,
